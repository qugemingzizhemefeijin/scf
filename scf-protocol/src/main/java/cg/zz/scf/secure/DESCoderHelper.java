 ...
 *
 */
public final class DESCoderHelper {
	...
	/**
	 * 生成8位byte密钥(系统自动生成) java默认密钥长度为56位
	 * 
	 * @return byte[] 8位byte
	 * @throws Exception
	 */
	...
	/**
	 * 生成密钥(自定义) 用于java环境c#下不可使用 使用 key 中的前 8 个字节作为 DES 密钥的密钥内容
	 * 
	 * @return
	 * @throws Exception
	 */
	...
	/**
	 * 解密 该方法与c#互通 data密文类型为base64加密后的字符串 key为密钥,本方法通过UTF-8方式获得key的byte类型
	 * 
	 * @param data - 密文
	 * @param key - 密钥(IV length must be 8 bytes long)
	 * @return 原文
	...
	/**
	 * 解密 该方法与c#互通
	 * data密文类型为原文加密后的byte类型,如果密文通过base64加密为字符串形式，解码请用方法decryptNetString
	 * (String data,String key)
	 * 如通过通过base64加密后字符串getByte方式调用该方法会抛出BadPaddingException: Given final
	 * block not properly padded 异常
