...
import cg.zz.scf.secure.Base64;
...
public class RSACoderHelper {
	...
	/**
	 * 加密(java与c#互通) java通过c#传来的公钥进行加密
	 * 
	 * @param data 原文
	 * @param module 模块
	 * @param exponent 指数
	...
	/**
	 * 解密(java与c#互通) java通过c#传来的私钥进行解密
	 * 
	 * @param data 密文
	 * @param delement 专用指数
	 * @param module 模块
	...
	private String getRSAPrivateKeyAsNetFormat(byte[] key) {
		try {
			...
			buff.append("<Modulus>" + b64encode(removeMSZero(pvkKey.getModulus().toByteArray()))
					+ "</Modulus>");
			buff.append("<Exponent>" + b64encode(removeMSZero(pvkKey.getPublicExponent().toByteArray()))
					+ "</Exponent>");// 公用指数
			...
			buff.append("<DP>" + b64encode(removeMSZero(pvkKey.getPrimeExponentP().toByteArray()))
					+ "</DP>");
			buff.append("<DQ>" + b64encode(removeMSZero(pvkKey.getPrimeExponentQ().toByteArray()))
					+ "</DQ>");
			buff.append("<InverseQ>" + b64encode(removeMSZero(pvkKey.getCrtCoefficient().toByteArray()))
					+ "</InverseQ>");
	...
	public static void main(String[] args) throws Exception {
		...
		// String miwen = rsa.encryptByPublicKeyString(inputstr,
		// publickey);
		...
		// System.out.println("私钥为:"+privatekey);
		// String miwen = encryptByPrivateKeyString(inputstr,
		// privatekey);
		// System.out.println("密文为:"+miwen);
